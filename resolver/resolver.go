...
func New(ctx context.Context, schema []byte, modifiers ...OptionModifier) (*Resolver, error) {
...
func (p *Resolver) SkipValidation() bool {
	return p.skipValidation
}

func (p *Resolver) createProviders(ctx context.Context) (providers []koanf.Provider, err error) {
...
func (p *Resolver) replaceKoanf(k *koanf.Koanf) {
	p.Koanf = k
}

func (p *Resolver) validate(k *koanf.Koanf) error {
	if p.skipValidation {
		return nil
	}
...
func (p *Resolver) SetTracer(_ context.Context, _ *otelx.Tracer) {
}

func (p *Resolver) runOnChanges(e watcherx.Event, err error) {
...
func (p *Resolver) reload(e watcherx.Event) {
	p.l.Lock()

	var err error
	defer func() {
		// we first want to unlock and then runOnChanges, so that the callbacks can actually use the Resolver
		p.l.Unlock()
...
func (p *Resolver) watchForFileChanges(ctx context.Context, c watcherx.EventChannel) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (p *Resolver) DirtyPatch(key string, value any) error {
	p.l.Lock()
	defer p.l.Unlock()
...
func (p *Resolver) Set(key string, value interface{}) error {
	p.l.Lock()
	defer p.l.Unlock()
...
func (p *Resolver) BoolF(key string, fallback bool) bool {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) StringF(key string, fallback string) string {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) StringsF(key string, fallback []string) (val []string) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) IntF(key string, fallback int) (val int) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) Float64F(key string, fallback float64) (val float64) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) DurationF(key string, fallback time.Duration) (val time.Duration) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) ByteSizeF(key string, fallback bytesize.ByteSize) bytesize.ByteSize {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) GetF(key string, fallback interface{}) (val interface{}) {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) CORS(prefix string, defaults cors.Options) (cors.Options, bool) {
	if len(prefix) > 0 {
...
func (p *Resolver) TracingConfig(serviceName string) *otelx.Config {
	return &otelx.Config{
...
func (p *Resolver) RequestURIF(path string, fallback *url.URL) *url.URL {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) URIF(path string, fallback *url.URL) *url.URL {
	p.l.RLock()
	defer p.l.RUnlock()
...
func (p *Resolver) PrintHumanReadableValidationErrors(w io.Writer, err error) {
	p.printHumanReadableValidationErrors(p.Koanf, w, err)
}

func (p *Resolver) printHumanReadableValidationErrors(k *koanf.Koanf, w io.Writer, err error) {
	if err == nil {
		return
	}
