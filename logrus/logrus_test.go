...
func TestJSONFormatter(t *testing.T) {
	t.Run("pretty=true", func(t *testing.T) {
		l := New("logrus-audit", "v0.0.0", ForceFormat("json_pretty"), ForceLevel(logrus.DebugLevel))
		var b bytes.Buffer
		l.Logrus().Out = &b
	...
	t.Run("pretty=false", func(t *testing.T) {
		l := New("logrus-audit", "v0.0.0", ForceFormat("json"), ForceLevel(logrus.DebugLevel))
		var b bytes.Buffer
		l.Logrus().Out = &b
...
func TestGelfFormatter(t *testing.T) {
	t.Run("gelf formatter", func(t *testing.T) {
		l := New("logrus-audit", "v0.0.0", ForceFormat("gelf"), ForceLevel(logrus.DebugLevel))
		var b bytes.Buffer
		l.Logrus().Out = &b
...
func TestTextLogger(t *testing.T) {
	audit := NewAudit("logrus-audit", "v0.0.0", ForceFormat("text"), ForceLevel(logrus.TraceLevel))
	tracer := New("logrus-app", "v0.0.0", ForceFormat("text"), ForceLevel(logrus.TraceLevel))
	debugger := New("logrus-server", "v0.0.1", ForceFormat("text"), ForceLevel(logrus.DebugLevel))
	warner := New("logrus-server", "v0.0.1", ForceFormat("text"), ForceLevel(logrus.WarnLevel))
	for k, tc := range []struct {
		...
		{
			l: audit,
			expect: []string{"logrus_test.go", "logrus_test.TestTextLogger",
				"audience=audit", "service_name=logrus-audit", "service_version=v0.0.0",
		...
		},
		{
			l: tracer,
			expect: []string{"logrus_test.go", "logrus_test.TestTextLogger",
				"audience=application", "service_name=logrus-app", "service_version=v0.0.0",
				"An error occurred.", "message:some error", "trace", "testing.tRunner"},
			call: func(l *Logger) {
				l.WithError(errors.New("some error")).Error("An error occurred.")
			},
		...
		{
			l: New("logrus-app", "v0.0.0", ForceFormat("text"), ForceLevel(logrus.TraceLevel), RedactionText("redacted")),
			expect: []string{"logrus_test.go", "logrus_test.TestTextLogger",
				"audience=application", "service_name=logrus-app", "service_version=v0.0.0",
				"An error occurred.", "headers:map[", "accept:application/json", "accept-encoding:gzip",
				"user-agent:Go-http-client/1.1", "x-request-id:id1234", "host:127.0.0.1:63232", "method:GET",
		...
		{
			l: New("logrus-server", "v0.0.1", ForceFormat("text"), LeakSensitive(), ForceLevel(logrus.DebugLevel)),
			expect: []string{
				"audience=application", "service_name=logrus-server", "service_version=v0.0.1",
				"An error occurred.",
			...
			notExpect: []string{"logrus_test.go", "logrus_test.TestTextLogger", "testing.tRunner", "?bar=foo"},
			call: func(l *Logger) {
				l.WithRequest(fakeRequest).Error("An error occurred.")
			},
		},
		...
		{
			l: debugger,
			expect: []string{"audience=application", "service_name=logrus-server", "service_version=v0.0.1",
		...
		},
		{
			...
			expect: []string{"audience=application", "service_name=logrus-server", "service_version=v0.0.1",
				"An error occurred.", "message:some error"},
			notExpect: []string{"logrus_test.go", "logrus_test.TestTextLogger", "trace", "testing.tRunner"},
			call: func(l *Logger) {
				l.WithError(errors.New("some error")).Error("An error occurred.")
			},
		},
		{
			l:         debugger,
			expect:    []string{"audience=application", "service_name=logrus-server", "service_version=v0.0.1", "baz!", "foo=bar"},
			notExpect: []string{"logrus_test.go", "logrus_test.TestTextLogger"},
			call: func(l *Logger) {
				l.WithField("foo", "bar").Info("baz!")
			},
		},
		{
			l: New("logrus-server", "v0.0.1", ForceFormat("text"), ForceLevel(logrus.DebugLevel)),
			expect: []string{
		...
		{
			l:      New("logrus-app", "v0.0.0", ForceFormat("text"), ForceLevel(logrus.TraceLevel), LeakSensitive()),
